    ...
    internal class ShowRFValuePlot
    {
        ...
        private (double, string )FindRFTestResult(List<RFTestResult> testResults, string str_testItem)
        {
            try
            {
                ...
                if (arraryHeadStr.Length == 5 &&
                    (arraryHeadStr[arraryHeadStr.Length - 1].Contains("EVM")
                    || arraryHeadStr[arraryHeadStr.Length - 1].Contains("FreqError")
                    || arraryHeadStr[arraryHeadStr.Length - 1].Contains("SymClkError")
                    || arraryHeadStr[arraryHeadStr.Length - 1] == "Power"
                    || arraryHeadStr[arraryHeadStr.Length - 1].Contains("LOLeakage")
                    || arraryHeadStr[arraryHeadStr.Length - 1].Contains("SpectrumMask")
                    ))
                {
                    temp = null;
                    bWifiTx = true;
                    ...
                    if (bWifiTx)
                    {
                        ...
                        //    result.Bandwidth.Contains(arraryHeadStr[2]) &&
                        //    result.Antenna.Contains(arraryHeadStr[3]) &&
                        ...
                        temp = testResults.FirstOrDefault(result =>
                                   ...
                                   result.DataRate.Contains(arraryHeadStr[1]) &&
                                   result.Bandwidth != null &&                // 检查Bandwidth是否为null
                                   result.Bandwidth.Contains(arraryHeadStr[2]) &&
                    ...
                    if (temp != null)
                    {
                        ...
                        if (arraryHeadStr[arraryHeadStr.Length - 1].Contains("FreqError"))
                        {
                        ...
                        if (arraryHeadStr[arraryHeadStr.Length - 1].Contains("SymClkError"))
                        {
                            if (double.TryParse(temp.Metrics_Tx.SymClkError, out double value))
                            {
                                dValue = value;
                            }
                            else
                            ...
                            sRange = temp.Metrics_Tx.SymClkError_Range;
                        }
                        ...
                        if (arraryHeadStr[arraryHeadStr.Length - 1].Contains("LOLeakage"))
                        {
                            if (double.TryParse(temp.Metrics_Tx.LOLeakage, out double value))
                            {
                                dValue = value;
                            }
                            else
                            ...
                            sRange = temp.Metrics_Tx.LOLeakage_Range;
                        }

                        if (arraryHeadStr[arraryHeadStr.Length - 1].Contains("SpectrumMask"))
                        {
                            if (double.TryParse(temp.Metrics_Tx.SpectrumMask, out double value))
                            {
                                dValue = value;
                            }
                            else
                            ...
                            sRange = temp.Metrics_Tx.SpectrumMask_Range;
                        }
                    }
                    ...
                    ))
                {
                    ...
                    if (temp != null)
                    {
                        ...
                        {
                            if (double.TryParse(temp.Metrics_Rx.PER, out double value))
                            {
                                dValue = value;
                            }
                            else
                ...
                (arraryHeadStr[arraryHeadStr.Length - 1] == "FreqOffset"
                || arraryHeadStr[arraryHeadStr.Length - 1] == "Power"
                ))
                {
                    temp = null;
                    ...
                    if (temp != null)
                    {
                        if (arraryHeadStr[arraryHeadStr.Length - 1].Contains("FreqOffset"))
                        {
                            if (double.TryParse(temp.Metrics_BtTx.FreqOffset, out double value))
                            {
                                dValue = value;
                            }
                            else
                            ...
                            sRange = temp.Metrics_BtTx.FreqOffset_Range;
                        }
                    ...
                    ))
                {
                    ...
                    if (temp != null)
                    {
                        if (arraryHeadStr[arraryHeadStr.Length - 1] == "Rx")
                        {
                            if (double.TryParse(temp.Metrics_BtRx.PER, out double value))
                            {
                                dValue = value;
                            }
                            else
        ...
        //方法重载，方便调用
        private void DrawLinechart(Plot myPlot, List<double> ys, ScottPlot.Color color, string str_Control_ItemName,string sRange, List<string> title = null)
        {
            DrawLinechart(myPlot, ys, color, str_Control_ItemName, sRange, null, title);
        }

        //原始方法
        private void DrawLinechart(Plot myPlot, List<double> ys, ScottPlot.Color color, string str_Control_ItemName, string sRange, List<double> xs = null, List<string> title = null)
        {
            bool flag = false;
            ...
            sp.LegendText= str_Control_ItemName + "_" + sRange;
