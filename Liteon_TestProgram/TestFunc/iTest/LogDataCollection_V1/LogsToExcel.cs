    ...
    internal class LogsToExcel
    {
        ...
        private void AppendDataToStreamWriter(RFTestResult result, StreamWriter sw, TestConfiguration LogCollectConfig)
        {
            ...
            if (result.Metrics_Tx != null)
            {
                ...
                //AppendMetricData(sb, result, "MaskErr", result.Metrics_Tx.MaskErr, result.Metrics_Tx.MaskErr_Range, result.Metrics_Tx.MaskErr_Result);
                if (LogCollectConfig.Wifi.SpectrumMask)
                {
                    string range = LogCollectConfig.Standard ? result.Metrics_Tx.MaskErr_Range : "EMPTY";
                    string resultText = LogCollectConfig.Result ? result.Metrics_Tx.MaskErr_Result : "EMPTY";

                    AppendMetricData(sb, result, "MaskErr", result.Metrics_Tx.MaskErr, range, resultText);
                }
            ...
            if (result.Metrics_BtTx != null)
            {
                ...
                //AppendMetricData(sb, result, "InitFreqErr", result.Metrics_BtTx.InitFreqErr, result.Metrics_BtTx.InitFreqErr_Range, result.Metrics_BtTx.InitFreqErr_Result);
                if (LogCollectConfig.Bluetooth.InitFreqErr)
                {
                    string range = LogCollectConfig.Standard ? result.Metrics_BtTx.InitFreqErr_Range : "EMPTY";
                    string resultText = LogCollectConfig.Result ? result.Metrics_BtTx.InitFreqErr_Result : "EMPTY";

                    AppendMetricData(sb, result, "InitFreqErr", result.Metrics_BtTx.InitFreqErr, range, resultText);
                }
            }
        ...
        private void AppendTitelToStreamWriter(RFTestResult result, StreamWriter sw, TestConfiguration LogCollectConfig)
        {
            ...
            if (result.Metrics_Tx != null)
            {
                ...
                if (LogCollectConfig.Wifi.EVM)
                {
                    string range = LogCollectConfig.Standard ? "EVM_Range" : "";
                    string resultText = LogCollectConfig.Result ? "EVM_Result" : "";

                    AppendMetricTitel(sb, result, "EVM", range, resultText);
                }

                //AppendMetricTitel(sb, result, "MaskErr", "MaskErr_Range", "MaskErr_Result");
                if (LogCollectConfig.Wifi.SpectrumMask)
                {
                    string range = LogCollectConfig.Standard ? "MaskErr_Range" : "";
                    string resultText = LogCollectConfig.Result ? "MaskErr_Result" : "";

                    AppendMetricTitel(sb, result, "MaskErr", range, resultText);
                }
            ...
            if (result.Metrics_BtTx != null)
            {
                ...
                //AppendMetricTitel(sb, result, "InitFreqErr", "InitFreqErr_Range", "InitFreqErr_Result");
                if (LogCollectConfig.Bluetooth.InitFreqErr)
                {
                    string range = LogCollectConfig.Standard ? "InitFreqErr_Range" : "";
                    string resultText = LogCollectConfig.Result ? "InitFreqErr_Result" : "";

                    AppendMetricTitel(sb, result, "InitFreqErr", range, resultText);
                }
            }
        ...
        private void AppendMetricTitel(StringBuilder sb, RFTestResult result, string metricNameValue1, string metricNameValue2, string metricNameValue3)
        {
            if (result.Metrics_Tx != null || result.Metrics_Rx != null)
            {
                ...
                {
                    sb.Append($"{result.Frequency}_{result.DataRate}_{result.Antenna}_{metricNameValue1},");
                    if (metricNameValue1.Contains("Rx_Power") == false)
                    {
                        UIHandleHelper.ControlHandle(comboBox_RFMode, () => { comboBox_RFMode.Items.Add($"{result.Frequency}#{result.DataRate}#{result.Antenna}#{metricNameValue1}"); });
                    }
                }

                if (string.IsNullOrEmpty(metricNameValue2) == false)
                {
                    sb.Append($"{result.Frequency}_{result.DataRate}_{result.Antenna}_{metricNameValue2},");
                }

                if (string.IsNullOrEmpty(metricNameValue3) == false)
                {
                    sb.Append($"{result.Frequency}_{result.DataRate}_{result.Antenna}_{metricNameValue3},");
                }
            }
