    ...
    internal class ShowRFValuePlot
    {
        ...
        private (double, string )FindRFTestResult(List<RFTestResult> testResults, string str_testItem)
        {
            try
            {
                ...
                if (arraryHeadStr.Length == 5 &&
                    ...
                    || arraryHeadStr[arraryHeadStr.Length - 1].Contains("RSSI")
                    ))
                {
                    temp = null;
                    bWifiRx = true;
                    ...
                    if (temp != null)
                    {
                        if (arraryHeadStr[arraryHeadStr.Length - 1].Contains("RSSI"))
                        {
                            if (double.TryParse(temp.Metrics_Rx.RSSI, out double value))
                            {
                                dValue = value;
                            }
                            else
                            ...
                            sRange = temp.Metrics_Rx.RSSI_Range;
                        }
                    }

                    if (temp != null)
                    {
                        if (arraryHeadStr[arraryHeadStr.Length - 1] == "Rx_Power")
                        {
                            if (double.TryParse(temp.Metrics_Rx.PER_Power, out double value))
                            {
                                dValue = value;
                            }
                            else
        ...
        public void PlotData(FormsPlot formsPlot, Dictionary<string, List<RFTestResult>> dict, string str_Control_ItemName, ScottPlot.Color color)
        {
            try
            {
                ...
                //List<double> yAxisLabels_Values
