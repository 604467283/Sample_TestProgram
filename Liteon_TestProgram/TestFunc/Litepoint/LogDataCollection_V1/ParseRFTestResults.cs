    ...
    internal class ParseRFTestResults
    {
        public (List<RFTestResult>, bool )ParseWifiTestResults(string filePath)
        {
            ...
            foreach (var line in File.ReadAllLines(filePath))
            {
                try
                {
                    if (line.Contains("TX_MULTI_VERIFICATION ") || line.Contains("RX_VERIFY_PER ") || ((line.Contains("TX_") || line.Contains("RX_")) && line.Contains(" __________")))
                    {
                        // 初始化新的测试结果对象
                        ...
                        //标题提取
                        if (line.Contains("TX_MULTI_VERIFICATION ") || line.Contains("RX_VERIFY_PER "))
                        {
                            string str_temp = "";
                            string str_FindMark1 = "";

                            if (line.Contains("TX_MULTI_VERIFICATION "))
                            {
                                str_FindMark1 = "TX_MULTI_VERIFICATION ";
                            }
                            else if (line.Contains("RX_VERIFY_PER "))
                            {
                                str_FindMark1 = "RX_VERIFY_PER ";
                            }
                        ...
                        //Bt
                        if ((line.Contains("TX_LE ") || line.Contains("TX_BDR ") || line.Contains("TX_EDR "))
                        ...
                        if ((line.Contains("RX_LE ") || line.Contains("RX_BDR ") || line.Contains("RX_EDR "))
                           && line.Contains(" __________")
                           )
                        {
                        ...
                        if ((line.Contains("TX_LE ") && line.Contains(" __________"))
                            || (line.Contains("TX_BDR ") && line.Contains(" __________"))
                             || (line.Contains("TX_EDR ") && line.Contains(" __________"))
                             || (line.Contains("RX_LE ") && line.Contains(" __________"))
                             || (line.Contains("RX_BDR ") && line.Contains(" __________"))
                             || (line.Contains("RX_EDR ") && line.Contains(" __________"))
                            )
                        {
                            string str_temp = "";
                    ...
                    else if (currentResult != null)
                    {
                        ...
                        string[] Arrary_WifiMark = { "EVM_AVG_DB", "FREQ_ERROR_AVG", "SYMBOL_CL", "POWER_AVG_DBM", "LO_LEAKAGE_",
                                                         "RX_POWER_DBM",  "PER",
