    ...
    internal class LogsToExcel
    {
        ...
        private void AppendDataToStreamWriter(RFTestResult result, StreamWriter sw, TestConfiguration LogCollectConfig)
        {
            ...
            if (result.Metrics_Rx != null)
            {
                ...
                //AppendMetricData(sb, result, "RSSI", result.Metrics_Rx.RSSI, result.Metrics_Rx.RSSI_Range, result.Metrics_Rx.RSSI_Result);
                if (LogCollectConfig.Wifi.RxRssi)
                {
                    string range = LogCollectConfig.Standard ? result.Metrics_Rx.RSSI_Range : "EMPTY";
                    string resultText = LogCollectConfig.Result ? result.Metrics_Rx.RSSI_Result : "EMPTY";

                    AppendMetricData(sb, result, "RSSI", result.Metrics_Rx.RSSI, range, resultText);
                }
            }
            ...
            if (result.Metrics_BtTx != null)
            {
                ...
                if (LogCollectConfig.Bluetooth.FreqOffset)
                {
                    ...
                    AppendMetricData(sb, result, "RSSI", result.Metrics_BtTx.FreqOffset, range, resultText);
                }
            ...
            if (result.Metrics_BtRx != null)
            {
                ...
                {
                    AppendMetricData(sb, result, "Power", result.Metrics_BtRx.PER_Power, "EMPTY", "EMPTY");
                }
        ...
        private void AppendTitelToStreamWriter(RFTestResult result, StreamWriter sw, TestConfiguration LogCollectConfig)
        {
            ...
            if (result.Metrics_Rx != null)
            {
               ...
               //AppendMetricTitel(sb, result, "Rx", "Rx_Range", "Rx_Result");
                ...
                //AppendMetricTitel(sb, result, "RSSI", "RSSI_Range", "RSSI_Result");
                if (LogCollectConfig.Wifi.RxRssi)
                {
                    string range = LogCollectConfig.Standard ? "RSSI_Range" : "";
                    string resultText = LogCollectConfig.Result ? "RSSI_Result" : "";

                    AppendMetricTitel(sb, result, "RSSI", range, resultText);
                }
            }
